...
class FyersService:
    ...
    # decrypt_token / encrypt_token removed - refresh tokens no longer supported.
    # Only plain access tokens are stored.

    async def save_tokens(self, access_token: str, db) -> dict:
        """Save only access token. Refresh token support removed."""
        from .token_service import save_access_token
        
        result = await save_access_token(access_token, db)
        return result

    # === REFRESH TOKEN / AUTO-RENEWAL REMOVED ===
    # (All refresh/auto-renewal methods removed in cleanup.)
    # Only manual access token workflow supported.
    # === END REMOVED SECTION ===
