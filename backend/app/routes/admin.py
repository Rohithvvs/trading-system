"""Admin routes (Sprint 2 user management + Sprint 3 feature permissions). Prefix: /admin"""
...
from ..services import admin_user_service, feature_permission_service

router = APIRouter()


def _to_user_response(user: User) -> UserAdminResponse:
...
# --- Sprint 3: Feature permissions (catalog only; not wired to product routes) ---
...
async def list_features(
    ...
    """Full feature permission catalog (active and inactive), ordered by feature_key."""
    _ = admin
...
async def update_feature_permission(
    feature_key: str,
    body: UpdateFeaturePermissionRequest,
    request: Request,
    admin: User = Depends(get_current_admin_user),
    db: AsyncSession = Depends(get_db),
):
    """Update allowed_roles / is_active / description with critical-feature safety."""
    ip_address = request.client.host if request.client else None
    user_agent = request.headers.get("user-agent")
    roles = list(body.allowed_roles) if body.allowed_roles is not None else None
    row = await feature_permission_service.update_feature_permission(
        db,
        actor=admin,
    ...
    return _to_feature_response(row)
