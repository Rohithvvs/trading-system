...
from backend.app.schemas.analysis import AnalysisMode, AnalysisRequest, ArticleItem, OHLCVPoint
...
async def test_orchestrator_service_integration(db_session, monkeypatch):
    ...
    news_articles = [
        ArticleItem(
            title="Good earnings beat expectations",
    ...
    with patch("backend.app.agents.fundamental_analysis_agent.yf.Ticker") as mock_ticker:
        ...
        # Must return ArticleItem instances — dicts break attribute access and shadow dedup.
        with patch(
            "backend.app.agents.news_analysis_agent.NewsService.fetch_recent_news"
        ) as mock_news:
            mock_news.return_value = news_articles
            with patch(
                "backend.app.services.sentiment_service.LLMService.analyze_sentiment"
            ) as mock_llm:
                ...
                # run_full is async — must be awaited
                response = await orchestrator.run_full(request)
