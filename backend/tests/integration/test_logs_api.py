"""System logs API integration tests (Sprint 5: auth + system_logs feature gate)."""
...
def _admin_headers(client: TestClient) -> dict:
    """Login via real async get_db path (do not use sync-overridden client fixture)."""
...
def test_logs_api_requires_auth(test_engine):
    """Sprint 5: /api/logs is gated — unauthenticated callers get 401."""
    with TestClient(app) as client:
        assert client.get("/api/logs?limit=1").status_code == 401


def test_logs_api_filters_by_level_and_symbol(db_session: Session, test_engine):
    # Use real async dependency graph (no get_db sync override)
    with TestClient(app) as client:
        ...
        response = client.get(
            "/api/logs?level=ERROR&symbol=HINDALCO-EQ", headers=headers
        )
...
def test_global_exception_handler_persists_zero_division_traceback(
    db_session: Session, test_engine
):
    with TestClient(app) as client:
        response = client.get(
            "/api/test/logs-zero-division",
            headers={"X-Correlation-ID": "cid-zero-div"},
        )
        ...
        log = (
            ...
            .filter(
                ...
                SystemLog.correlationId == "cid-zero-div",
            )
...
def test_logs_websocket_receives_realtime_broadcast(test_engine):
    with TestClient(app) as client:
